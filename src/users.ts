...
class FetchUser {
    ...
    constructor(query?: string) {
        const queryString = QueryString.stringify({query: query}, {encode: false})
...
export default class Users{
    ...
    async fetchBySlug(slug: string): Promise<any> {
        const queryString = QueryString.stringify({filter: {slug: slug}}, {encode: false})
        try {
    ...
    async fetchSelf (auth: KitsuAuthToken): Promise<any> {
        const queryString = QueryString.stringify({filter: {self: true}}, {encode: false})
